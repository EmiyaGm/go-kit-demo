...
func NewHTTPHandler(endpoints endpoint.Set, tracer stdopentracing.Tracer, logger log.Logger) http.Handler {
	...
	m.Handle("/uppercase", httptransport.NewServer(
		...
		append(options, httptransport.ServerBefore(opentracing.HTTPToContext(tracer, "Uppercase", logger)))...,
	))
	return m
}
...
func NewHTTPClient(instance string, tracer stdopentracing.Tracer, logger log.Logger) (service.Service, error) {
